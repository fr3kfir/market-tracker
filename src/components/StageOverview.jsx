...
const CustomTooltip = ({ active, payload, label, theme }) => {
  ...
  return (
    <div
      className="rounded p-2 text-xs font-mono shadow-lg"
      style={{
        background: isLight ? '#ffffff' : '#0c1220',
        border: `1px solid ${isLight ? '#d0d7de' : '#1a2540'}`,
        color: isLight ? '#1f2328' : undefined,
      }}
    >
      <div style={{ color: isLight ? '#636c76' : '#9ca3af', marginBottom: '4px' }}>{label}</div>
      {payload.map(p => (
...
export default function StageOverview({ distribution, history, totalStocks = 5600, theme }) {
  const isLight = theme === 'light';
  const tickFill = isLight ? '#636c76' : '#3a4a60';
  ...
  return (
    <div className="panel h-full">
      ...
      <div className="text-xs mb-2" style={{ color: 'var(--text-muted)' }}>Market Overview</div>
        ...
        {Object.entries(distribution).map(([stage, pct]) => (
          <div key={stage} className="text-center">
            <div className="flex items-center justify-center gap-1 mb-0.5">
              ...
              />
              <span className="text-xs" style={{ color: 'var(--text-muted)' }}>{STAGE_LABELS[stage]}</span>
            </div>
      ...
      <div className="flex gap-3 text-xs mb-2">
        {Object.entries(STAGE_COLORS).map(([s, c]) => (
          <span key={s} className="flex items-center gap-1" style={{ color: 'var(--text-muted)' }}>
